...
import GetTailwindColor from "../components/GetTailwindColor"
...
export default function OverviewPots() {
    ...
    if (isLoading) {
        return (
            <section id = "overview-pots" className = 'w-[clamp(300px, 90%, 400px)]'>
            <div className = "flex justify-between">
                <h3 className = 'text-typ3'>Pots</h3>
                <div>See Details</div>
            </div>
            <div className='flex flex-col tablet:flex-row gap-4 w-full p-4'>
                <div className = 'flex items-center p-4 rounded-md gap-4 '>
                ...
                <div className='grid grid-cols-2 gap-4'>
                    <LeftBar borderColor = 'bg-green'>
                        <Skeleton className = 'w-full h-full' />
                    </LeftBar>
                    <LeftBar borderColor = 'bg-cyan'>
                        <Skeleton className = 'w-full h-full' />
                    </LeftBar>
                    <LeftBar borderColor = 'bg-navy'>
                        <Skeleton className = 'w-full h-full' />
                    </LeftBar>
                    <LeftBar borderColor = 'bg-yellow'>
                        <Skeleton className = 'w-full h-full' />
                    </LeftBar>
                </div>
            </div>
    ...
    if (error || !pots) {
        return (
            <div>Error loading pots</div>
        )
    }

    const totalSaved = pots.reduce((sum, pot) => sum + (pot.total || 0), 0)
    const sortedPots = pots.sort((a, b) => (b.total || 0) - (a.total || 0))
    return (
        <section id = "overview-pots" className = 'w-[clamp(300px, 90%, 400px)]'>
            <div className = "flex justify-between">
                <h3 className = 'text-typ3'>Pots</h3>
            ...
            <div className='flex flex-col tablet:flex-row gap-4 w-full p-4'>
                ...
                <div className='grid grid-cols-2 gap-4'>
                    {sortedPots.slice(0, 4).map(pot => (
                        <LeftBar borderColor = {GetTailwindColor(pot.theme)} key = {pot.id}>
                            <p>{pot.name}</p>
                            <p>${pot.total}</p>
                        </LeftBar>
                    ))}
